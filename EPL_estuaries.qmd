...
# Set folder path to the intergrated ecosystem map (IEM) 
...
# Add tabular data (in excel change sheet name to "Data", and remove last two rows of NAs, change cols with numbers at start of name)
...
# set folder path - cant use list.files on gdbs -  this code first looks at all folders ending in gdb then finds the one you want 
pa_dirs <- list.dirs(
  path = Sys.getenv("NBA_DATA"),
  recursive = TRUE,
  full.names = TRUE)

pa_fp <- grep("NLC_change\\.gdb$", pa_dirs, value = TRUE)

# Add spatial data - use PA MArch 2025 then extract Estuaries
pa <-st_read(pa_fp, layer = "SANBI_PA_2024Q4_Mar2025") 
